...
function measure(label, fn) {
    ...
    /* this is a memory leak; enable only if needed!
        ...
        if (attrName.charAt(0) === '@') {
            ...
            w(expression, (value) => {
                ...
                value !== false
                    ...
                    // remove the attribute as soon as it's safe
                    : toRemove ? toRemove.push(attrName) : node.removeAttribute(attrName);
            });
        }
    }
    toRemove.forEach((attrName) => node.removeAttribute(attrName));
    toRemove = null; // no more deferred attribute removal after the loop
}
/**
